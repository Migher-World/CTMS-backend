...
export abstract class AbstractController {
  ...
  create(@Body() payload: any) {
    return resolveResponse(this.service.create(payload), `${this.service.modelName} Created`);
  }
  ...
  update(@Param('id') id: string, @Body() payload: any) {
    return resolveResponse(this.service.update(id, payload), `${this.service.modelName} Updated`);
  }

  // @ApiResponse({ status: 200, type: AbstractResponse })
  @Delete(':id')
  remove(@Param('id') id: string) {
    return resolveResponse(this.service.remove(id), `${this.service.modelName} Deleted`);
  }
}
